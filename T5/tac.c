...
{
	fprintf(stderr, "\n================================\nCódigo Intermediário Gerado abaixo:\n================================\n");
...
TAC* tacGenerateCode(Ast *node)
{
	...
	switch(node->type)
	{
		...
		case AST_IF: 		result = makeIfThen(code[0], code[1], code[2]); break;

		case AST_UNTIL: 		result = makeWhile(code[0], code[1]); break;
		case AST_PRINTLIST: 		result = tacJoin(tacJoin(code[0], tacCreate(TAC_PRINT, code[0]?code[0]->res:0, 0, 0)), code[1]); break;

		case AST_CALL_FUNCTION: return tacJoin(code[1], tacCreate(TAC_CALL,makeTemp(),node->son[0]->symbol,0)); break;
